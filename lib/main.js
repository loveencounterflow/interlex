(function() {
  ...
  Token = class Token {
    ...
    constructor(cfg) {
      var ref, ref1;
      debug('Ω___4', "new Token", cfg.name, cfg.level, cfg.level.grammar);
      this.name = cfg.name;
    ...
    parse_jump(jump_literal) {
      ...
      if ((match = jump_literal.match(_jump_spec_re)) == null) {
        throw new Error(`Ω___5 expected a well-formed jump literal, got ${rpr(jump_literal)}`);
      }
      ref = match.groups;
      for (key in ref) {
        level_name = ref[key];
      ...
      if (level_name === '.') {
        ...
        throw new Error(`Ω___6 expected name of a known level, got ${rpr(level_name)}`);
      }
  ...
  Lexeme = class Lexeme {
    ...
    constructor(token, match) {
      var ref;
      // debug 'Ω___7', token
      // debug 'Ω___8', token.jump, token.grammar.levels[ token.jump.level ] if token.jump?
    ...
    * [Symbol.iterator]() {
    ...
    new_token(cfg) {
      ...
      if ((cfg.level != null) && cfg.level !== this) {
        throw new Error("Ω___9 inconsistent level");
      }
  ...
  Grammar = class Grammar {
    ...
    new_level(cfg) {
      ...
      if (this.levels[cfg.name] != null) {
        throw new Error(`Ω__10 level ${rpr(level.name)} elready exists`);
      }
      level = new Level({
        ...cfg,
        grammar: this
    ...
    tokenize(source) {
      ...
      while (true) {
        ...
        info('Ω__12', f`${start}:>3.0f;:${stop}:<3.0f; ${fqname}:<20c; ${rpr(hit)}:<30c; ${jump_rpr}:<15c; ${groups_rpr}`);
        start = stop;
      }
      return null;
    }
