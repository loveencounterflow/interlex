(function() {
  ...
  Token = class Token {
      ...
      hide(this, 'data_merge_strategy', (function() {
        switch (true) {
          ...
          case isa(std.function, this.merge):
            return 'call';
          default:
            throw new Error(`Ωilx__11 expected a valid input for \`merge\`, got ${rpr(this.merge)}`);
        }
      }).call(this));
      return void 0;
    }
    ...
    static _parse_jump(spec, level = null) {
      var action, carry, fit, i, len, match, ref, target;
      if (spec == null) {
        return null;
      }
      match = null;
  ...
  Grammar = class Grammar {
    ...
    _new_error_signal(kind, start, stop, source, message) {
      var R;
      R = this._new_signal('error', start, source, {kind, message});
      R.stop = stop;
      R.hit = source.slice(start, +stop + 1 || 9e9);
      return R;
    }

    //---------------------------------------------------------------------------------------------------------
    _new_jump_signal(start, source, from_level, to_level) {
      return this._new_signal('jump', start, source, {from_level, to_level});
    }
...
//.......................................................................................................
        ...
        switch (true) {
          ...
          case lexeme.fqname === '$signal.stop':
            if (lexeme.stop !== last_idx) {
              yield this._new_error_signal('earlystop', lexeme.stop, last_idx, source, `expected stop at ${last_idx}, got ${rpr(lexeme.stop)}`);
            }
            break;
          //...................................................................................................
          ...
          case is_first && (lexeme.start !== 0):
            yield this._new_error_signal('latestart', 0, lexeme.start, source, `expected start at 0, got ${rpr(lexeme.start)}`);
        }
        //.....................................................................................................
        yield lexeme;
        is_first = false;
    ...
    * _scan_3_merge(source) {
      ...
      merge_data_as_lists = function(merged, lexemes) {
        var R, i, key, len, lexeme, ref, value;
      ...
      flush = function*() {
        ...
        switch (merged.token.data_merge_strategy) {
          ...
          case 'list':
            ...
            break;
          default:
            throw new Error(`Ωilx__11 should never happen: encountered data_merge_strategy == ${rpr(merged.token.data_merge_strategy)}`);
        }
        yield merged;
        active_fqname = null;
        lexemes.length = 0;
...
//.......................................................................................................
    ...
    * _scan_4_startstop_lnr(source) {
      ...
      yield this._new_signal('stop', (ref = prv_lexeme != null ? prv_lexeme.stop : void 0) != null ? ref : 0, source);
      this.state.lnr++;
      return null;
    }
