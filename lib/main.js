    ...
    * [Symbol.iterator]() {
    ...
    match_at(start, source) {
      ...
      if (this.positions.has(start)) {
        ...
        if (this.grammar.cfg.loop_errors === 'emit') {
      ...
      switch (this.strategy) {
        ...
        case 'longest':
          ...
          break;
        default:
          throw new Error(`Ωilx__11 should never happen: got strategy: ${rpr(this.strategy)}`);
      }
      if (lexeme == null) {
        //.......................................................................................................
      ...
      throw new Error(`Ωilx__12 encountered zero-length match for token ${rpr(fqname)} at position ${lexeme.start} ${quote}`);
    }
  ...
  Grammar = class Grammar {
    ...
    new_level(cfg) {
      ...
      if (this.levels[cfg.name] != null) {
        throw new Error(`Ωilx__13 level ${rpr(level.name)} elready exists`);
      }
      level = new Level({
        ...cfg,
        grammar: this
        ...
        switch (true) {
          ...
          case lexeme.fqname === '$signal.stop':
            if (lexeme.stop !== last_idx) {
              yield this._new_error_signal('Ωilx__14', 'earlystop', lexeme.stop, last_idx, source, `expected stop at ${last_idx}, got ${rpr(lexeme.stop)}`);
            }
            break;
          //...................................................................................................
          ...
          case is_first && (lexeme.start !== 0):
            yield this._new_error_signal('Ωilx__15', 'latestart', 0, lexeme.start, source, `expected start at 0, got ${rpr(lexeme.start)}`);
        }
        //.....................................................................................................
        yield lexeme;
        is_first = false;
      ...
      flush = function*() {
        ...
        switch (merged.token.data_merge_strategy) {
          ...
          case 'list':
            ...
            break;
          default:
            throw new Error(`Ωilx__16 should never happen: encountered data_merge_strategy == ${rpr(merged.token.data_merge_strategy)}`);
        }
        yield merged;
        active_fqname = null;
        lexemes.length = 0;
...
//.......................................................................................................
    ...
    * _scan_4_match_tokens(source) {
        ...
        if ((jump = lexeme.jump) != null) {
          switch (jump.action) {
            ...
            case 'back':
              ...
              break;
            default:
              throw new Error(`Ωilx__17 should never happen: unknown jump action ${rpr(lexeme.jump.action)}`);
          }
          if (jump.carry) {
            jump_before = true;
    ...
    _get_level(level_name) {
      ...
      throw new Error(`Ωilx__18 unknown level ${rpr(level_name)}`);
    }
