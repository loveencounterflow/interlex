(function() {
  'use strict';
  var Grammar, Internals, Level, Levelstack, Lexeme, Token, clone, create, debug, hide, hide_getter, info, internals, isa, isa_optional, new_regex_tag, rpr, rx, set_getter, std, validate, validate_optional;

  //===========================================================================================================
  ({Levelstack, hide, hide_getter, set_getter, clone, debug, info, rpr} = require('./helpers'));
    ...
    * [Symbol.iterator]() {
    ...
    match_at(start, source) {
      ...
      if (this.positions.has(start)) {
        /* TAINT show source */
        throw new Error(`Ωilx___9 encountered loop at position ${rpr(start)} ${rpr(this.positions)}`);
      }
  ...
  Grammar = class Grammar {
    ...
    _new_jump_signal(action, start, source, from_level, to_level) {
      return this._new_signal('jump', start, source, {action, from_level, to_level});
    }
    ...
    * _scan_1b_merge_jumps(source) {
      ...
      for (lexeme of this._scan_2_validate_exhaustion(source)) {
        ...
        jump.assign({
          to_level: last_jump.data.to_level
        });
        buffer.length = 0;
        yield jump;
        yield lexeme;
        ...
        switch (true) {
          ...
          case lexeme.fqname === '$signal.stop':
            if (lexeme.stop !== last_idx) {
              yield this._new_error_signal('Ωilx__12', 'earlystop', lexeme.stop, last_idx, source, `expected stop at ${last_idx}, got ${rpr(lexeme.stop)}`);
            }
            break;
          //...................................................................................................
          ...
          case is_first && (lexeme.start !== 0):
            yield this._new_error_signal('Ωilx__13', 'latestart', 0, lexeme.start, source, `expected start at 0, got ${rpr(lexeme.start)}`);
        }
        //.....................................................................................................
        yield lexeme;
        is_first = false;
      ...
      flush = function*() {
        ...
        switch (merged.token.data_merge_strategy) {
          ...
          case 'list':
            ...
            break;
          default:
            throw new Error(`Ωilx__14 should never happen: encountered data_merge_strategy == ${rpr(merged.token.data_merge_strategy)}`);
        }
        yield merged;
        active_fqname = null;
        lexemes.length = 0;
...
//.......................................................................................................
    ...
    * _scan_4_match_tokens(source) {
      ...
      old_level_name = null;
      //.......................................................................................................
      yield this._new_jump_signal('fore', 0, source, null, this.start_level.name);
      while (true) {
        //.......................................................................................................
        level = stack.peek();
        new_level = level;
        ...
        if ((jump = lexeme.jump) != null) {
          switch (jump.action) {
            ...
            case 'back':
              ...
              break;
            default:
              throw new Error(`Ωilx__19 should never happen: unknown jump action ${rpr(lexeme.jump.action)}`);
          }
          if (jump.carry) {
            jump_before = true;
        ...
        //.....................................................................................................
        if (jump_before) {
          yield this._new_jump_signal(jump.action, lexeme.start, source, level.name, lexeme.level.name);
        }
        yield lexeme;
        if (jump_after) {
          yield this._new_jump_signal(jump.action, start, source, lexeme.level.name, new_level.name);
        }
      }
      //.......................................................................................................
      while (!stack.is_empty) {
        yield this._new_jump_signal('back', start, source, stack.pop_name(null), stack.peek_name(null));
      }
      return null;
    }
