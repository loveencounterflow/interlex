(function() {
  'use strict';
  var Grammar, Internals, Level, Levelstack, Lexeme, Token, clone, create, create_pod_from_template, debug, hide, hide_getter, info, internals, isa, isa_optional, new_regex_tag, quote_source, rpr, rx, set_getter, std, type_of, validate, validate_optional;

  //===========================================================================================================
  // insert_position_marker
  ({Levelstack, hide, hide_getter, set_getter, clone, quote_source, create_pod_from_template, debug, info, rpr} = require('./helpers'));
  ...
  Lexeme = class Lexeme {
    ...
    constructor(token, match) {
      ...
      set_getter(this, 'is_error', () => {
        return this.token.level.name === '$error';
      });
  ...
  Level = class Level {
    ...
    constructor(cfg) {
      ...
      hide(this, 'tokens', [...((ref4 = cfg.tokens) != null ? ref4 : [])]);
  ...
  Grammar = class Grammar {
    ...
    constructor(cfg) {
      ...
      hide_getter(this, 'has_error', function() {
    ...
    reset_data(data = null) {
      ...
      if ((data === null) || (data === true)) {
        ...
        this.assign(this.data, this.cfg.data, data);
      }
      ref1 = this.data;
      for (key in ref1) {
        fn = ref1[key];
        if (isa(std.function, fn)) {
          (this.data[key] = fn.call(this));
        }
      }
      return null;
    }
      ...
      for (lexeme of this._scan_3_merge(source)) {
        switch (true) {
          ...
          case lexeme.fqname === '$signal.stop':
            if (lexeme.stop !== last_idx) {
              ...
              switch (this.cfg.earlystop_errors) {
                case 'emit':
                  yield this._new_error_signal('Ωilx__17', 'earlystop', lexeme.stop, last_idx, source, `expected stop at ${last_idx}, got ${rpr(lexeme.stop)}`);
                  break;
                case 'throw':
          ...
          case is_first && (lexeme.start !== 0):
            yield this._new_error_signal('Ωilx__19', 'latestart', 0, lexeme.start, source, `expected start at 0, got ${rpr(lexeme.start)}`);
        }
        //.....................................................................................................
        yield lexeme;
        is_first = false;
      ...
      flush = function*() {
        ...
        switch (merged.token.data_merge_strategy) {
          ...
          case 'list':
            ...
            break;
          default:
            throw new Error(`Ωilx__20 should never happen: encountered data_merge_strategy == ${rpr(merged.token.data_merge_strategy)}`);
        }
        yield merged;
        active_fqname = null;
        lexemes.length = 0;
        ...
        if ((jump = lexeme.jump) != null) {
          switch (jump.action) {
            ...
            case 'back':
              ...
              break;
            default:
              throw new Error(`Ωilx__21 should never happen: unknown jump action ${rpr(lexeme.jump.action)}`);
          }
          if (jump.carry) {
            jump_before = true;
