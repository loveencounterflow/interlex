(function() {
  'use strict';
  var Grammar, Internals, Level, Levelstack, Lexeme, Token, clone, create, debug, hide, hide_getter, info, internals, isa, isa_optional, new_regex_tag, quote_source, rpr, rx, set_getter, std, type_of, validate, validate_optional;
  ...
  ({type_of, std, isa, isa_optional, create, validate, validate_optional} = require('cleartype'));
      ...
      hide(this, 'data_merge_strategy', (function() {
        switch (true) {
          ...
          case isa(std.function, this.merge):
            return 'call';
          default:
            throw new Error(`Ωilx___6 expected a valid input for \`merge\`, got ${rpr(this.merge)}`);
        }
      }).call(this));
      return void 0;
    }
  ...
  Lexeme = class Lexeme {
    ...
    constructor(token, match) {
      ...
      hide(this, 'emit', this.emit.bind(this));
      //.......................................................................................................
    ...
    emit(fqname, start, source, data = null) {
      ...
      lexeme.assign(data);
      /* TAINT use API */
      this.token.grammar.state.emitted_lexemes.unshift(lexeme);
      return lexeme;
    }
  ...
  Level = class Level {
    ...
    constructor(cfg) {
      ...
      this.cast = (ref3 = cfg.cast) != null ? ref3 : null;
  ...
  Grammar = class Grammar {
    ...
    * scan(source) {
      ...
      this.state.stack = new Levelstack();
      yield* (function() {
        switch (true) {
      ...
      for (lexeme of this._scan_3_merge(source)) {
        switch (true) {
          ...
          case lexeme.fqname === '$signal.stop':
            if (lexeme.stop !== last_idx) {
              ...
              switch (this.cfg.earlystop_errors) {
                case 'emit':
                  yield this._new_error_signal('Ωilx__23', 'earlystop', lexeme.stop, last_idx, source, `expected stop at ${last_idx}, got ${rpr(lexeme.stop)}`);
                  break;
                case 'throw':
          ...
          case is_first && (lexeme.start !== 0):
            yield this._new_error_signal('Ωilx__25', 'latestart', 0, lexeme.start, source, `expected start at 0, got ${rpr(lexeme.start)}`);
        }
        //.....................................................................................................
        yield lexeme;
        is_first = false;
      ...
      flush = function*() {
        ...
        switch (merged.token.data_merge_strategy) {
          ...
          case 'list':
            ...
            break;
          default:
            throw new Error(`Ωilx__26 should never happen: encountered data_merge_strategy == ${rpr(merged.token.data_merge_strategy)}`);
        }
        yield merged;
        active_fqname = null;
        lexemes.length = 0;
...
//.......................................................................................................
      for (lexeme of this._scan_4_startstop_lnr_TMP(source)) {
...
//.......................................................................................................
      for (lexeme of this._scan_4_startstop_lnr(source)) {
        switch (true) {
          ...
          case lexeme.fqname === '$signal.start':
            yield lexeme;
            prv_level_name = this.start_level.name;
            yield this._new_jump_signal(0, source, prv_level_name);
            break;
          //...................................................................................................
          case lexeme.fqname === '$signal.stop':
            prv_level_name = null;
            yield this._new_jump_signal(lexeme.start, source, prv_level_name);
            yield lexeme;
            break;
          //...................................................................................................
          case lexeme.is_user:
            if (lexeme.token.level.name !== prv_level_name) {
              prv_level_name = lexeme.token.level.name;
              yield this._new_jump_signal(lexeme.start, source, prv_level_name);
            }
            if (lexeme.level.name !== prv_level_name) {
              prv_level_name = lexeme.level.name;
              yield this._new_jump_signal(lexeme.start, source, prv_level_name);
            }
    ...
    * _scan_4_startstop_lnr(source) {
      ...
      for (lexeme of this._scan_4_apply_casts(source)) {
        if (!lexeme.is_signal) {
          prv_lexeme = lexeme;
        }
        yield lexeme;
    ...
    * _scan_4_apply_casts(source) {
      var lexeme;
      for (lexeme of this._scan_5_match_tokens(source)) {
        if (lexeme.is_user) {
          switch (true) {
            case lexeme.token.cast != null:
              lexeme.token.cast.call(this, lexeme);
              break;
            case lexeme.level.cast != null:
              lexeme.level.cast.call(this, lexeme);
              break;
            case this.cast != null:
              this.cast.call(this, lexeme);
          }
        }
        while (this.state.emitted_lexemes.length > 0) {
          /* TAINT use API */
          yield this.state.emitted_lexemes.pop();
        }
        yield lexeme;
      }
      return null;
    ...
    * _scan_5_match_tokens(source) {
      var jump, level, lexeme, new_level, old_level_name, stack, start;
      start = 0;
      lexeme = null;
      old_level_name = null;
        ...
        if ((jump = lexeme.jump) != null) {
          switch (jump.action) {
            ...
            case 'back':
              ...
              break;
            default:
              throw new Error(`Ωilx__30 should never happen: unknown jump action ${rpr(lexeme.jump.action)}`);
          }
          if (jump.carry) {
    ...
    _get_level(level_name) {
      ...
      throw new Error(`Ωilx__31 unknown level ${rpr(level_name)}`);
    }
