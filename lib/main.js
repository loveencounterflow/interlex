(function() {
  ...
  Level = class Level {
    ...
    constructor(cfg) {
      ...
      hide(this, 'tokens', [...((ref3 = cfg.tokens) != null ? ref3 : [])]);
    ...
    match_at(start, source) {
      ...
      if ((!this.is_system) && this.positions.has(start)) {
        ...
        switch (this.grammar.cfg.loop_errors) {
          case 'emit':
            return this.grammar._new_error_signal('Ωilx___9', 'loop', start, start, source, message);
          case 'throw':
            throw new Error(`Ωilx__10 ${message}`);
          default:
            throw new Error(`Ωilx__11 should never happen: got unknown value for loop_errors: ${rpr(this.grammar.cfg.loop_errors)}`);
        }
      }
      ...
      switch (this.strategy) {
        ...
        case 'longest':
          ...
          break;
        default:
          throw new Error(`Ωilx__12 should never happen: got strategy: ${rpr(this.strategy)}`);
      }
      if (lexeme == null) {
        //.......................................................................................................
      ...
      for (lexeme of this._scan_3_merge(source)) {
        switch (true) {
          ...
          case lexeme.fqname === '$signal.stop':
            if (lexeme.stop !== last_idx) {
              ...
              switch (this.cfg.earlystop_errors) {
                case 'emit':
                  yield this._new_error_signal('Ωilx__16', 'earlystop', lexeme.stop, last_idx, source, `expected stop at ${last_idx}, got ${rpr(lexeme.stop)}`);
                  break;
                case 'throw':
          ...
          case is_first && (lexeme.start !== 0):
            yield this._new_error_signal('Ωilx__18', 'latestart', 0, lexeme.start, source, `expected start at 0, got ${rpr(lexeme.start)}`);
        }
        //.....................................................................................................
        yield lexeme;
        is_first = false;
      ...
      flush = function*() {
        ...
        switch (merged.token.data_merge_strategy) {
          ...
          case 'list':
            ...
            break;
          default:
            throw new Error(`Ωilx__19 should never happen: encountered data_merge_strategy == ${rpr(merged.token.data_merge_strategy)}`);
        }
        yield merged;
        active_fqname = null;
        lexemes.length = 0;
        ...
        if ((jump = lexeme.jump) != null) {
          switch (jump.action) {
            ...
            case 'back':
              ...
              break;
            default:
              throw new Error(`Ωilx__20 should never happen: unknown jump action ${rpr(lexeme.jump.action)}`);
          }
          if (jump.carry) {
            jump_before = true;
