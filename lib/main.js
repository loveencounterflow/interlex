(function() {
  ...
  Grammar = class Grammar {
    ...
    //---------------------------------------------------------------------------------------------------------
    * walk_lexemes(source) {
      yield* this._walk_lexemes(source);
      this.state.lnr++;
      return null;
    }
    ...
    * _walk_lexemes(source) {
      var jump, jump_after_lexeme, jump_before_lexeme, level, lexeme, new_level, old_level_name, stack, start;
      start = 0;
      ...
      while (true) {
        ...
        if (jump_before_lexeme) {
          yield this._new_jump_signal(start, source, level.name, lexeme.level.name);
        }
        yield lexeme;
