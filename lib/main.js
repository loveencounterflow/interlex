(function() {
  'use strict';
  var Grammar, Level, Lexeme, Stack, Token, _jsid_re, _jump_spec_back, _jump_spec_re, _valid_regex_flags_re, debug, hide, info, new_regex_tag, partial, regex, rpr, rx;
  ...
  new_regex_tag = function(flags = 'dy') {
    ...
    return new Proxy(R, {
      get: function(target, key) {
        ...
        if (!_valid_regex_flags_re.test(local_flags)) {
  ...
  Token = class Token {
    ...
    static _parse_jump(jump_spec) {
      ...
      if ((match = jump_spec.match(_jump_spec_re)) == null) {
        throw new Error(`Ωilx___4 expected a well-formed jump literal, got ${rpr(jump_spec)}`);
      }
    ...
    * [Symbol.iterator]() {
    ...
    new_token(cfg) {
      ...
      if ((cfg.level != null) && cfg.level !== this) {
        throw new Error("Ωilx___5 inconsistent level");
      }
  ...
  Grammar = class Grammar {
    ...
    new_level(cfg) {
      ...
      if (this.levels[cfg.name] != null) {
        throw new Error(`Ωilx___6 level ${rpr(level.name)} elready exists`);
      }
      level = new Level({
        ...cfg,
        grammar: this
  ...
  */
  module.exports = {Token, Lexeme, Level, Grammar, regex, rx, new_regex_tag, _jsid_re, _jump_spec_re, _valid_regex_flags_re};
