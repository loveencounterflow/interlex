(function() {
  ...
  Grammar = class Grammar {
    ...
    * walk_lexemes(source) {
      ...
      if (!this.cfg.simplify_jumps) {
        yield* this._walk_lexemes_1(source);
        return null;
      }
      //.......................................................................................................
      /* Consolidate all contiguous jump signals into single signal: */
      buffer = [];
      for (lexeme of this._walk_lexemes_1(source)) {
    ...
    * _walk_lexemes_1(source) {
      yield* this._walk_lexemes_2(source);
      this.state.lnr++;
      return null;
    }
    ...
    * _walk_lexemes_2(source) {
      var emit_signals, jump, jump_after, jump_before, level, lexeme, new_level, old_level_name, stack, start;
      start = 0;
      stack = new Levelstack(this.start_level);
      lexeme = null;
      emit_signals = this.cfg.emit_signals;
      old_level_name = null;
      //.......................................................................................................
      if (emit_signals) {
        ...
        if ((jump = lexeme.jump) != null) {
          switch (jump.action) {
            ...
            case 'back':
              ...
              break;
            default:
              throw new Error(`Ωilx__14 should never happen: unknown jump action ${rpr(lexeme.jump.action)}`);
          }
          if (jump.carry) {
            if (emit_signals) {
              jump_before = true;
      ...
      if (emit_signals) {
        ...
        yield this.system_tokens.stop.match_at(start, source);
      }
      return null;
    }
    ...
    _get_level(level_name) {
      ...
      throw new Error(`Ωilx__15 unknown level ${rpr(level_name)}`);
    }
