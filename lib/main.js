(function() {
  ...
  Grammar = class Grammar {
    ...
    _add_system_level() {
      var $system;
      $system = this.new_level({
        name: '$system'
      });
      hide(this, 'system_tokens', {
        start: $system.new_token({
          name: 'start',
          matcher: /|/
        }),
        stop: $system.new_token({
          name: 'stop',
          matcher: /|/
        }),
        jump: $system.new_token({
          name: 'jump',
          matcher: /|/
        }),
        error: $system.new_token({
          name: 'error',
          matcher: /|/
        })
      });
    ...
    * walk_lexemes(source) {
      ...
      var jump, level, lexeme, new_level, old_level_name, signal, stack, start;
      start = 0;
      ...
      while (true) {
        ...
        //.....................................................................................................
        if (this.cfg.emit_signals && (lexeme.level.name !== old_level_name)) {
          signal = this.system_tokens.jump.match_at(start, source);
      ...
      //.......................................................................................................
      if (this.cfg.emit_signals) {
        if ((new_level != null) && (old_level_name !== new_level.name)) {
          /* TAINT do this in API call */
          signal = this.system_tokens.jump.match_at(start, source);
          signal.data.from_level = old_level_name;
        ...
        while (!stack.is_empty()) {
          ...
          signal.data.to_level = stack.is_empty() ? null : stack.peek().name;
          yield signal;
        }
