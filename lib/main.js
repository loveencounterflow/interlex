(function() {
  'use strict';
  var Grammar, Internals, Level, Levelstack, Lexeme, Token, Typespace, clone, create, debug, hide, hide_getter, ilx, info, internals, isa, isa_optional, new_regex_tag, quote_source, rpr, rx, set_getter, std, std2, type_of, validate, validate_optional;
  ...
  Grammar = class Grammar {
    //---------------------------------------------------------------------------------------------------------
    constructor(cfg) {
      var cast, cast_method, cfg_template;
      cfg_template = {
        name: 'g',
        strategy: 'first',
        emit_signals: true,
...
//.......................................................................................................
    ...
    * _scan_6_insert_startstop_lnr(source) {
      var current_stop, lexeme;
      current_stop = 0;
      yield this._new_signal('start', 0, source);
      for (lexeme of this._scan_7_apply_casts(source)) {
        if (lexeme.is_user) {
          current_stop = lexeme.stop;
        }
        yield lexeme;
      }
      yield this._new_signal('stop', current_stop, source);
      ...
      for (lexeme of this._scan_8_match_tokens(source)) {
        ...
        switch (cast_owner.cast_method) {
          ...
          case 'walk':
            ...
            break;
          default:
            throw new Error(`Ωilx__27 should never happen: got unknown cast_method ${rpr(cast_owner.cast_method)}`);
        }
      }
      return null;
    }
    ...
    * _scan_8_match_tokens(source) {
      ...
      goto_token = null;
      if ((goto_token = this.state.current_token) != null) {
        /* TAINT just push start_level and token.level to stack? */
        if (goto_token.level !== (last_level = stack.peek(null))) {
          throw new Error(`Ωilx__14 expected level of ${goto_token.fqname} on stack, found ${(ref1 = last_level != null ? last_level.name : void 0) != null ? ref1 : 'nothing'}`);
        }
      } else {
        ...
        if ((jump = lexeme.jump) != null) {
          switch (jump.action) {
            ...
            case 'back':
              ...
              break;
            default:
              throw new Error(`Ωilx__28 should never happen: unknown jump action ${rpr(lexeme.jump.action)}`);
          }
          if (jump.carry) {
